...
export const getProfessionalApplications = async (page = 1, limit = 10) => {
  const { data } = await axiosInstance.get(
    `/application/status/candidate/Referral?page=${page}&limit=${limit}`,
  );
  return data;
};
...
export const getReferredByMe = async (page: number = 1, limit: number = 10) => {
  const response = await axiosInstance.get(
