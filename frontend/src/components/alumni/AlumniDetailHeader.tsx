...
export default function AlumniDetailHeader({
  ...
  const currentExperience =
    profile.experiences?.find((exp) => exp.isCurrent || !exp.endDate) ||
    profile.experiences?.[0];

  const currentRole = currentExperience?.role || "Professional";
  const college =
    profile.educations?.find((e) => e.isCurrent)?.college ||
              ...
              <div className="flex flex-col items-end gap-2 flex-shrink-0">
                ...
                <div className="flex items-center gap-2 w-full">
                  <button
                    onClick={() =>
                      router.push(
                        `/${userType}/message/${profile.userId}?userName=${encodeURIComponent(
                          profile.name || "User",
